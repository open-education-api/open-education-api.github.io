...
!> When a client requests programmes or courses and the query parameter `consumer` is set to `eduxchange`, only Programs and Courses meant for eduxchange should be returned.
...
General attributes
  ...
  * `themes` (v2.1): an array of themes of the Program or Course within the alliance, `["theme 1", "theme 2"]`. If the themes attribute is present for a particular course or programme, theme is ignored. If theme is avaiable, but themes is not, the theme attribute is used. If both are missing, the course or programme has no themes.
  * `selection` (v2.0): boolean value (`true` or `false`) indicating whether this Program or Course is selective, e.g. whether student need to pass extra requirements before being allowed to enrol.
...
Attributes regarding visibility and enrolment of different types of users. Eduxchange can be used by students and professionals. Please note, there are three types of students. (1) Students from the alliance partner institutions,  (2) students from the own institution and (3) guest students from institutions outside the alliance.
  ...
  * `visibleForOwnStudents` (v2.0): a boolean value (`true` or `false`) indicating whether enrolment of a Program or Course should be visible for students of the offering institution. *Note: in the eduxchange frontend a higher level institution setting is set to indicate that programmes and courses themselves are visible for own students.*
  * `enrolmentForOwnStudents` (v2.0): a string indicating which enrolments process should be followed for students of the offering institution. Allowed values are `"broker"` or `"url"`. This attribute is only used if `visibleForOwnStudents` is set to `true`. 
    * If `"url"` is chosen the attribute `enrolmentUrl` **in the consumer object of an offering** is mandatory.
  * `visibleForGuests` (v2.1): a boolean value (`true` or `false`) indicating whether enrolment of a Program or Course should be visible for students outside the partner institutions. *Note: in the eduxchange frontend a higher level institution setting is set to indicate that programmes and courses themselves are visible for students outside the partner institutions.*
  * `enrolmentForGuests` (v2.1): a string indicating which enrolments process should be followed for students outside the partner institutions. Allowed values are `"broker"` or `"url"`. This attribute is only used if `visibleForGuests` is set to `true`. 
    * If `"url"` is chosen the attribute `enrolmentUrlForGuests` **in the consumer object of an offering** is mandatory.
  * `enrolmentForProfessionals` (v2.2): a string indicating which enrolments process should be followed for professionals. Allowed values are `"broker"` or `"url"`. This attribute is only used if `targetGroup` is set to `forProfessionals`. 
...
Attributes regarding joint programmes.
  ...
  * `jointPartnerCodes` (v2.1): an array of partners of the Program. This is used to identify the partners in case of a joint programme. The agreed partner codes are used here. For example in the `lde` alliance: `["21PF", "21PB"]`.
  * `source` (v2.0): an optional object with a reference to the source of a Course or Program. In case of a joint programme one of the institutions could act as overall coordinator and specifies the programme and underlying courses. Underlying courses could be given at one of the other institutions. In this source object the course at the other institution can be specified. Use these attributes:
...
This consumer object is used to specify the `enrolmentUrl` and `enrolmentUrlForGuests` per offering. These attributes are associated with the corresponding attributes in the consumer object of the programme or course.
  ...
  * `enrolmentUrl` (v2.0): a string formatted as an URL to which own students will be redirected if `enrolmentForOwnStudents` **in the programme/course consumer object** is set to `"url"`.
  * `enrolmentUrlForGuests` (v2.1): a string formatted as an URL to which guest students will be redirected if `enrolmentForGuests` **in the programme/course consumer object** is set to `"url"`.
  * `enrolmentUrlForProfessionals` (v2.2): a string formatted as an URL to which professionals will be redirected if `enrolmentForProfessioanals` **in the programme/course consumer object** is set to `"url"`.
...
* `enrolments` (v2.0), an array with all the CROHO enrolments for this person. Each enrolment is an object with the following attributes:
  ...
  * `startDate` (v2.0): the start date for this enrolment. Should be a string formatted as an RFC3099 full-date.
  * `endDate` (v2.0): end start date for this enrolment. Should be a string formatted as an RFC3099 full-date.
