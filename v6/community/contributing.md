...
- Links should have descriptive names (e.g. `author` instead of `person` for
  blog posts).
...
A release branch:
- is named `release/<major>.<minor>` (for example `release/6.1`);
...
2. **Creating a maintained version**
   - When a version requires long-term maintenance, a release branch
     (`release/x.y`) is created from `main`.

3. **Bug fixes**
   - Start on the lowest maintained release branch.
   - Are forward-merged to newer release branches if applicable.
   - Are finally forward-merged into `main`.
...
- `main` is the single source of truth for the highest working version.
